...
The app is static files with ES modules, so it needs to be served rather than opened from `file://`:

    npm start

then open http://localhost:8000. The quals (see AGENTS.md) run with:

    npm run qual

`flowchart.js` is the transcription of the flowchart as a graph; `quals/flowchart.qual.js` holds the golden plain-text table it must match. The source image is in `reference/flowchart.png`.
